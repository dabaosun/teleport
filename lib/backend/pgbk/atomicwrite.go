		...
		for _, ca := range condacts {
			...
			case backend.KindRevision:
				...
				condBatch.Queue(
					...
					fmt.Printf("---> Revision(%s@%s) -> %v\n", ca.Key, ca.Condition.Revision, cond)
					conditionFailed = conditionFailed || !cond
					return nil
				})
			default:
			...
			case backend.KindPut:
				...
				actBatch.Queue(
					...
					nonNil(ca.Action.Item.Key), nonNil(ca.Action.Item.Value), zeronull.Timestamptz(ca.Action.Item.Expires.UTC()), newRevision,
				)
